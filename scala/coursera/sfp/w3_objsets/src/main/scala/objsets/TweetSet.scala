...
class Tweet(val user: String, val text: String, val retweets: Int):
  ...
  def moreRetweeted(that: Tweet): Tweet = if retweets >= that.retweets then this else that
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def unionAcc(acc: TweetSet): TweetSet =
    acc.union(left).incl(elem).union(right)

  def mostRetweeted: Tweet =
    val m1 = elem
    val m2 = if left.isEmpty then m1 else m1 moreRetweeted left.mostRetweeted
    val m3 = if right.isEmpty then m2 else m2 moreRetweeted right.mostRetweeted
    m3
...
object GoogleVsApple:
  ...
  lazy val googleTweets: TweetSet = allTweets.filter(t => google.exists(kw => t.text.contains(kw)))
  lazy val appleTweets: TweetSet = allTweets.filter(t => apple.exists(kw => t.text.contains(kw)))
