...
-- solution
newtype CoroutineT m a = CoroutineT { runCoroutineT :: (a -> m ()) -> m () }
...
instance Monad m => Monad (CoroutineT m) where
  ...
  (CoroutineT cx) >>= k = CoroutineT $ \c -> cx (\x -> runCoroutineT (k x) c)

type LogCoroutine = CoroutineT (Writer [String])

evalCorEx :: Show a => LogCoroutine a -> ((), [String])
evalCorEx m = runWriter (runCoroutineT m c)
  where c x = do { tell [show x]; return () }

coroutineEq :: Show a => LogCoroutine a -> LogCoroutine a -> Bool
coroutineEq m1 m2 = evalCorEx m1 == evalCorEx m2

kleisliEx :: a -> CoroutineT m a
kleisliEx x = CoroutineT $ \c -> c x

kleisliEx2 :: Enum a => a -> CoroutineT m a
kleisliEx2 x = CoroutineT $ \c -> c (succ x)

kleisliEx3 :: Enum a => a -> CoroutineT m a
kleisliEx3 x = CoroutineT $ \c -> c (succ $ succ x)

leftIdLaw :: Show a => a -> (a -> LogCoroutine a) -> Bool
...
rightIdLaw :: Show a => CoroutineT (Writer [String]) a -> Bool
rightIdLaw m = coroutineEq (m >>= return)  m

rightIdLawTest :: Bool
rightIdLawTest = rightIdLaw (kleisliEx 'a')

assocLaw :: Show c => LogCoroutine a -> (a -> LogCoroutine b) -> (b -> LogCoroutine c) -> Bool
...
runCoroutines (CoroutineT c1) (CoroutineT c2)
  = c1 $ \cont1 -> _
